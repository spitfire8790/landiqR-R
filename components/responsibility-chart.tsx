...
import { Users, Clock, BarChart3, PieChart, TrendingUp, Activity, UserCircle, Timer, ChartBar, PieChart as PieChartIcon, TrendingUp as TrendingUpIcon } from "lucide-react"
import type { Person, Allocation, Task, Responsibility, TaskAllocation } from "@/lib/types"
import { fetchTasks, fetchResponsibilities, fetchTaskAllocations } from "@/lib/data-service"
...
export default function ResponsibilityChart({ people, allocations }: ResponsibilityChartProps) {
  ...
  const getOrgColor = (org: string, alpha: number = 1) => {
    ...
    return orgColors[org as keyof typeof orgColors] || `rgba(107, 114, 128, ${alpha})`
  }
  ...
  const responsibilityAllocationData = useMemo(() => {
    ...
    allocations.forEach(allocation => {
      responsibilityCounts[allocation.personId] = (responsibilityCounts[allocation.personId] || 0) + 1
    })
    
    const sortedPeople = [...people]
      .filter(person => responsibilityCounts[person.id] > 0)
      .sort((a, b) => (responsibilityCounts[b.id] || 0) - (responsibilityCounts[a.id] || 0))
      ...
      datasets: [
        {
          ...
          data: sortedPeople.map(person => responsibilityCounts[person.id] || 0),
          backgroundColor: sortedPeople.map(person => getOrgColor(person.organisation, 0.8)),
          borderColor: sortedPeople.map(person => getOrgColor(person.organisation, 1)),
          borderWidth: 2,
          borderRadius: 8,
          borderSkipped: false,
        },
  ...
  const taskTimeAllocationData = useMemo(() => {
    ...
    taskAllocations.forEach(allocation => {
      ...
      if (task && task.hoursPerWeek) {
        timeByPerson[allocation.personId] = (timeByPerson[allocation.personId] || 0) + task.hoursPerWeek
      }
    })
  ...
  const organisationData = useMemo(() => {
    ...
    people.forEach(person => {
      orgCounts[person.organisation] = (orgCounts[person.organisation] || 0) + 1
    })
  ...
  const workloadTrendData = useMemo(() => {
    ...
    taskAllocations.forEach(allocation => {
      ...
      if (person && task && task.hoursPerWeek) {
        workloadByOrg[person.organisation] = (workloadByOrg[person.organisation] || 0) + task.hoursPerWeek
      }
    })
  ...
  const stats = useMemo(() => {
    ...
    const avgHoursPerPerson = totalPeople > 0 ? (totalWeeklyHours / totalPeople).toFixed(1) : '0'
    const tasksWithAssignments = tasks.filter(task => 
      taskAllocations.some(allocation => allocation.taskId === task.id)
    ).length
    const assignmentRate = totalTasks > 0 ? ((tasksWithAssignments / totalTasks) * 100).toFixed(1) : '0'
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">Comprehensive insights into team allocation and workload</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="bg-white border border-black rounded-lg">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-gray-700 text-sm font-medium">Total People</p>
            ...
            <Card className="bg-white border border-black rounded-lg">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-gray-700 text-sm font-medium">Weekly Hours</p>
                    <p className="text-3xl font-bold">{stats.totalWeeklyHours}</p>
                  </div>
            ...
            <Card className="bg-white border border-black rounded-lg">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-gray-700 text-sm font-medium">Avg Hours/Person</p>
                    <p className="text-3xl font-bold">{stats.avgHoursPerPerson}</p>
                  </div>
            ...
            <Card className="bg-white border border-black rounded-lg">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-gray-700 text-sm font-medium">Assignment Rate</p>
                    <p className="text-3xl font-bold">{stats.assignmentRate}%</p>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="bg-white border border-black rounded-lg">
              <CardHeader className="pb-4">
                ...
                <CardDescription>Number of category responsibilities assigned to each team member</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-80">
                  <Bar data={responsibilityAllocationData} options={chartOptions as any} />
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-white border border-black rounded-lg">
              <CardHeader className="pb-4">
                ...
                <CardDescription>Weekly hours allocated to each person across all tasks</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-80">
                  <Bar data={taskTimeAllocationData} options={chartOptions as any} />
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-white border border-black rounded-lg">
              <CardHeader className="pb-4">
                ...
                <CardDescription>Breakdown of team members across organisations</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-80">
                  <Doughnut data={organisationData} options={doughnutOptions as any} />
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-white border border-black rounded-lg">
              <CardHeader className="pb-4">
                ...
                <CardDescription>Total weekly hours allocated by organisation</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-80">
                  <Line data={workloadTrendData} options={{
                    ...chartOptions,
                    scales: {
                      ...chartOptions.scales,
                      y: {
          ...
          <Card className="bg-white border border-black rounded-lg">
            <CardHeader>
              <CardTitle className="text-lg font-semibold text-gray-900">Organisation Legend</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex flex-wrap gap-4">
                {Array.from(new Set(people.map(p => p.organisation))).map(org => (
                  <div key={org} className="flex items-center space-x-2">
                    ...
                    <Badge variant="secondary" className="text-xs">
                      {people.filter(p => p.organisation === org).length} members
                    </Badge>
                  </div>
                ))}
              </div>
